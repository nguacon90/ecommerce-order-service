...
public class RuleOrderEntryPercentageDiscountRAOActionTest {
    private RuleOrderEntryPercentageDiscountRAOAction action;
    ...
    public void setUp() {
        ...
        action = new RuleOrderEntryPercentageDiscountRAOAction();
        ...
        when(orderEntryRAO.getCurrencyIsoCode()).thenReturn("VND");
        when(cartRAO.getEntries()).thenReturn(entries);
        when(this.ruleEngineCalculationService.addOrderEntryLevelDiscount(any(), eq(false), any(BigDecimal.class))).thenReturn(discountRAO);
        when(this.ruleEngineCalculationService.addOrderEntryLevelDiscountWithConsumableQty(any(), anyBoolean(), any(BigDecimal.class), anyInt())).thenReturn(discountRAO);
        ...
        when(discountRAO.getFiredRuleCode()).thenReturn("fixedEntryDiscountCode");
        ruleEngineResultRAO.setActions(new LinkedHashSet<>());
        when(context.getValue(RuleEngineResultRAO.class)).thenReturn(ruleEngineResultRAO);
        when(context.getRuleEngineResultRao()).thenReturn(ruleEngineResultRAO);
        when(discountRAO.getConsumedEntries()).thenReturn(new HashSet<>());
        when(cartRAO.getEntries()).thenReturn(entries);
    }
    ...
    public void performAction_withNotConfigMaximumQuantityToApplied() {
        ...
        verify(this.ruleEngineCalculationService).addOrderEntryLevelDiscount(eq(orderEntryRAO), eq(false), any(BigDecimal.class));
    }
    ...
    public void performAction_withConfigMaximumQuantityToAppliedIsOne_HasNotCoupon() {
        ...
        verify(this.ruleEngineCalculationService, times(1)).addOrderEntryLevelDiscountWithConsumableQty(eq(orderEntryRAOMock2),
                anyBoolean(), any(), consumableQuantityEntry2Captor.capture());
    ...
    public void performAction_MaximumQuantity1_With3Entries_2EntriesConsumedAll_HasNotCoupon() {
        ...
        verify(this.ruleEngineCalculationService).addOrderEntryLevelDiscountWithConsumableQty(eq(orderEntryRAOMock3),
                eq(false), any(), consumableQuantityEntry3Captor.capture());
